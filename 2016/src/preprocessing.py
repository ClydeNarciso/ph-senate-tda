"""Precinct-level cleaning for the 2016 election data.

Changes vs 2022
---------------
- OAV pseudo-province set is 2016-specific: ASIA, EUROPE,
  MIDDLE EAST AND AFRICAS, NORTH AND LATIN AMERICA.
  There are no non-OAV region pseudo-provinces to drop (all four
  overseas labels appear only under Region == 'OAV').
- No NON_PROVINCE_LABELS to drop (unlike 2025's LAV / SGA rows).
...
def preprocess_and_clean_data(df_raw: pd.DataFrame,
    ...
    df_report.to_csv(TABLES_DIR / '2016_outlier_report.csv', index=False)
    with open(TABLES_DIR / '2016_outlier_report.tex', 'w') as f:
        f.write(
            df_report.style
            .format({'% Removed': '{:.2f}\\%'})
            .hide(axis="index")
