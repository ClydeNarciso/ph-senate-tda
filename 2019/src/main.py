"""
Main execution script for the 2019 electoral-topology / socioeconomic
analysis pipeline.

Reads precinct-level features from data/processed/ and socioeconomic
sources from "data/socioeconomic data/", runs the TDA + BallMapper
pipeline, and writes:
  - figures       -> results/figures/
  - cached/intermediate computations -> results/persistence_cache/
...
def main():
    # -------------------------------------------------------------------
    # 1. Pipeline extraction: clean data -> H0 intervals -> 4D features
    # -------------------------------------------------------------------
    ...
    # Merge socioeconomic data (poverty, ethnicity, IRA dependency, dynasty/GINI)
    df_features = merge_socioeconomic_data(
        ...
        ari_csv_path=config.ARI_DEPENDENCY_FILE,
    ...
    color_configs = {
        'Admin_Share': {
            ...
            'vmin': 0.0, 'vmax': 1.0, 'label': 'Average Admin Vote Share',
            'format': lambda val, loc: f'{val*100:.0f}%', 'threshold': 0.5,
        },
        'Poverty': {
            'col': '2018 Poverty Incidence', 'cmap': 'RdYlBu_r',
            'vmin': None, 'vmax': None, 'label': 'Mean 2018 Poverty Incidence',
            'format': lambda val, loc: f'{val:.1f}%', 'threshold': 16.6,
        },
        'Ethnicity': {
            'col': 'Dominant_Dialect', 'cmap': None,
            'vmin': None, 'vmax': None, 'label': 'Node Majority Ethnicity',
            'format': None, 'threshold': None,
        },
        'Dynasty_HHI': {
            'col': 'HHI', 'cmap': 'viridis',
            'vmin': None, 'vmax': None, 'label': 'Mean 2019 HHI (Dynasty Concentration)',
            'format': lambda val, loc: f'{val:.2f}', 'threshold': None,
        },
        'Inequality_GINI': {
            'col': 'GINI', 'cmap': 'plasma',
            'vmin': None, 'vmax': None, 'label': 'Mean 2019 GINI Coefficient',
            'format': lambda val, loc: f'{val:.3f}', 'threshold': None,
        },
    }
    ...
    for mode in target_modes:
        ...
        export_summary_to_latex(df_bm_summary, config.TABLES_DIR / f"summary_table_{mode}.tex", cfg['col'])
    ...
    if last_bm_instance is not None:
        ...
        provincial_topo_path = config.TABLES_DIR / 'provincial_topological_summaries_all.csv'
        topo_export_cols = [
            'Province', 'Region',
            'H0_PersistentEntropy', 'H0_L2_Norm', 'H0_Total_Persistence',
            'H0_PersistentEntropy_scaled', 'H0_L2_Norm_scaled', 'H0_Total_Persistence_scaled',
        ]
        ...
        print(f"-> Saved Provincial Topological Summaries to: {provincial_topo_path.name}")
        ...
        for node in nodes:
            ...
            mode_dialect = df_pts['Dominant_Dialect'].mode().iloc[0] if not df_pts.empty else 'N/A'
            total_elements = len(df_pts)
            maj_count = int(df_pts['Dominant_Dialect'].value_counts().iloc[0]) if total_elements > 0 else 0

            prov_region_list = [f"{row['Province']} ({row['Region']})" for _, row in df_pts.iterrows()]
            prov_dialect_list = [f"{row['Province']} ({row['Dominant_Dialect']})" for _, row in df_pts.iterrows()]

            node_row = {
                'Node ID': node,
                'No. Provinces': total_elements,
                'Provinces Included': ', '.join(df_pts['Province'].tolist()),
                'Provinces (Region)': ', '.join(prov_region_list),
                'No. of Provinces in each Region': ', '.join(f"{reg} ({cnt})" for reg, cnt in df_pts['Region'].value_counts().items()),
                ...
                'Mean 2018 Poverty Incidence': df_pts['2018 Poverty Incidence'].mean() if '2018 Poverty Incidence' in df_pts.columns else np.nan,
                'Mean HHI': df_pts['HHI'].mean() if 'HHI' in df_pts.columns else np.nan,
                'Mean GINI': df_pts['GINI'].mean() if 'GINI' in df_pts.columns else np.nan,
                'Majority Dominant Dialect': mode_dialect,
                'Major Dialect Ratio': f"{maj_count}/{total_elements}",
                'Provinces (Dialect)': ', '.join(prov_dialect_list),
            }
            master_nodes_data.append(node_row)
        ...
        print(f"-> Saved Comprehensive Master Summary Table to: {master_csv_path.name}")

    # -------------------------------------------------------------------
    # 6. Comparative epsilon-robustness (high vs. low covariate subgroups)
    # -------------------------------------------------------------------
    run_epsilon_comparative_robustness(
        df_data=df_features,
        target_col='2018 Poverty Incidence',
