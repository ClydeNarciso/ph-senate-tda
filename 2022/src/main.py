"""
Main execution script for the 2022 electoral-topology / socioeconomic
analysis pipeline.
...
pipeline, and writes:
  ...
  - intermediate cache  -> results/persistence_cache/
  - summary tables      -> results/tables/

Data notes
----------
- Poverty  : 2021 survey year (closest prior to 2022 election).
             Maguindanao del Norte/Sur gaps filled from 2023 data.
- IRA      : FY2022 XLSX (actual 2022 data available).
- Dynasty  : 2019 proxy (no 2022 data exists in the dynasty dataset).
- Ethnicity: 2020 PSA census (same source as 2019 pipeline).
"""
import warnings
...
def main():
    ...
    #    Poverty label comes from config.POVERTY_COL_LABEL ("2021 Poverty Incidence")
    # -------------------------------------------------------------------
    pov_label = config.POVERTY_COL_LABEL
    ...
    # -------------------------------------------------------------------
    target_modes = [
        'Admin_Share', 'Poverty', 'Ethnicity', 'Dynasty_HHI', 'Inequality_GINI'
    ]
    ...
    if last_bm_instance is not None:
        ...
        for node in nodes:
            ...
            master_nodes_data.append({
                ...
                f'Mean {pov_col}': (
                    df_pts[pov_col].mean() if pov_col in df_pts.columns else np.nan
                ),
